...
K1 = sense_ref_mv * (R1 + R2) / R2  # Should equal BATTERY_ADC_FULL_SCALE_MV = 15000
print(f"sense_ref_mv = {sense_ref_mv:.2f} mV")
print(f"K1 = {K1:.2f} mV (should be ~15000)\n")
...
# The formula is: voltage = ((adc - adc_at_0v) * K1) / (adc_at_15v - adc_at_0v)
# Which expands to: voltage = adc * K1/(adc_at_15v - adc_at_0v) - adc_at_0v * K1/(adc_at_15v - adc_at_0v)
# Comparing to: voltage = slope_mm * adc + intercept_mm
# We have:
#   slope_mm = K1 / (adc_at_15v - adc_at_0v)
#   intercept_mm = -adc_at_0v * K1 / (adc_at_15v - adc_at_0v)
#
# Solving:
#   adc_at_15v - adc_at_0v = K1 / slope_mm
#   adc_at_0v = -intercept_mm / slope_mm

optimal_adc_at_0v = -int(round(intercept_mm / slope_mm))
adc_range = K1 / slope_mm
optimal_adc_at_15v = optimal_adc_at_0v + int(round(adc_range))

print(f"\n\n=== Proposed Optimal Settings ===")
print(f"Recommended adcAt0V: {optimal_adc_at_0v}")
print(f"Recommended adcAt15V: {optimal_adc_at_15v}")
print(f"(ADC range: {optimal_adc_at_15v} - ({optimal_adc_at_0v}) = {optimal_adc_at_15v - optimal_adc_at_0v} counts)")

# Calculate errors with proposed settings
def voltage_from_adc_proposed(adc_raw):
    if optimal_adc_at_15v == optimal_adc_at_0v:
