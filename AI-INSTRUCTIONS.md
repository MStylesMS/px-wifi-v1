...
- Hardware must not block boot: `hwOk` / `hwFault` + red `.hw-banner` on every
  page when a required peripheral is missing. Shared contract:
  [../px-components/docs/hw-fault.md](../px-components/docs/hw-fault.md).
